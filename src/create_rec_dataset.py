...
DSET_IMGS_FOLDER = os.path.join(os.getcwd(), "dataset_output", "images")
...
# SCHOOL_NICKNAME(s) are available in 'res/schools/schools_*language*.json'
...
if __name__ == "__main__":
    ...
    (
        ...
        x_displacement,
        y_displacement,
    ) = sr.get_table_displacements(num_students=STUDENTS)
    (
        train_alpha_numeric_proportion,
        eval_alpha_numeric_proportion,
        alpha_num,
    ) = sr.get_alpha_numeric_proportion(num_students=STUDENTS)
    ...
    for school in schools_list:
        if school["nickname"] == SCHOOL_NICKNAME:
            break

    head_studies = sr.create_head_studies_profile(
    ...
    for student in tqdm(range(STUDENTS)):
        ...
        pdf_instance.body_features_setter(texts_to_include=texts_to_include)
        ...
        for academic_year in ACADEMIC_YEARS:
            ...
            if JUST_TABLE:
                grades_per_year = pdf_instance.body_just_table(
                    subjects_semantic,
                    x_displacement[student],
                    y_displacement[student],
                    alpha_num[student],
                )
                ...
                pdf_instance.move_abscissa(displacement=5)
                grades_per_year = pdf_instance.body(
                    subjects_semantic,
                    x_displacement[student],
                    y_displacement[student],
                    alpha_num[student],
            ...
            grades_per_subject_metrics = sr.update_grades_per_subject_metrics(
                metrics=grades_per_subject_metrics, new_grades=grades_per_year[0]
            )
            dictionary = pdf_instance.png_dict_getter()
            list_of_dicts.append(dictionary)

        pdf_name = os.path.join(
            PDF_DOCS_FOLDER, "".join([str(student).zfill(DOCS_Z_FILL), ".pdf"])
        )
        ...
        for i, image in enumerate(images):
            ...
            try:
                ...
                sr.save_layout(
                    img=image_copy, img_name=png_number, path=pngs_folder, dict=dict
                )
                sr.generate_json(
                    json_path=pngs_folder, json_number=png_number, dict=dict
                )
            ...
            # Save the image
            image_path_funsd_fashion = os.path.join(
                DSET_IMGS_FOLDER, "".join([id, ".png"])
            )
            try:
                ...
                sr.generate_json(
                    json_path=DSET_ANNOTATIONS_FOLDER, json_number=id, dict=dict
                )
                # Save the '.json' with ground truth
                sr.generate_json(
                    json_path=DSET_GROUND_TRUTH_FOLDER,
                    json_number=id,
                    dict=grades_per_year[i],
                )
    ...
    sr.generate_json(
        json_path=os.getcwd(), json_number="dataset_metrics", dict=dataset_metrics
    )
